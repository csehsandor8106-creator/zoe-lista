(() => {
  ...
  const RULES = [
    // FŰSZEREK
    {aliases:['chili cayenne','chilli cayenne','cayenne','cayenne bors','cayenne pepper'],label:'Cayenne bors',category:'Alapélelmiszer',icon:'🌶️',price:699,unit:'csomag',legacyDbToDefault:true},
    ...
    {aliases:['oregánó','oregano','szurokfű','szurokfu'],label:'Oregánó',category:'Alapélelmiszer',icon:'🌿',price:599,unit:'csomag',legacyDbToDefault:true},
  ...
  // Régi Egyéb/db scanner-találatok javítása úgy, hogy a részletes online név megmaradjon.
  let state = [];
  ...
  for (const item of state) {
    ...
    if (item.unit === rule.unit && (!Number(item.price) || Number(item.price) === 699 || originalName !== rule.label)) {
